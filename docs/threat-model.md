...
| T15 | SSRF / redirect-chain abuse via a malicious or compromised source | A4, A6 | `VerificationRequest.max_redirects` (default 3) and `redirect_chain_exceeded` flagging bound hop count; Gate D has no actual network/fetch code at all, so no request currently executes against any URL |
