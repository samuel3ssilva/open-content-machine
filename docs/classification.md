...
| `BI` (bare abbreviation) | *(not mapped)* | — | — | Deliberately left unexpanded (too ambiguous as a 2-letter token); only the full phrase `business intelligence`/`inteligência de negócios` is mapped. `Analista de BI` therefore falls to the generic low "analista" rule, not a forced high match. |
...
| `Apaixonado por Tecnologia` | engineering_data_ai | — | high | **Not** a bug: this non-conventional title genuinely contains the domain keyword "tecnologia" and correctly resolves at T2. Contrast with truly domain-empty non-conventional titles ("Resolvedor de Problemas", "Fazedor de Coisas"), which stay `unknown`. |
