"""Anthropic provider stub (ADR 0002 §6).
...
class AnthropicProvider:
    """Stub provider for Anthropic models. Performs no network I/O this sprint."""
    ...
    def is_available(self) -> bool:
        """True only if an API key is set and the ``anthropic`` SDK is installed."""
        if not self._settings.anthropic_api_key:
            return False
        try:
            import anthropic  # noqa: F401  (lazy availability probe only)
        except ImportError:
            return False
        return True
    ...
    def complete(self, request: ModelRequest) -> ModelResponse:
        ...
        if not self.is_available():
            raise ProviderNotConfiguredError(
                "Anthropic provider is not configured. Set ANTHROPIC_API_KEY in "
                ".env and install the 'anthropic' extra. Never paste keys into "
                "the CLI or chat."
            )
